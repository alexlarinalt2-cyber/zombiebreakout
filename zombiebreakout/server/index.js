...
function startGame(room){
  ...
  // Send game:started FIRST so client initialises (sets mpMode, clears zs).
  // Then send wave:zombies so it lands after the client is ready and won't be wiped.
  io.to(room.id).emit('game:started',{difficulty:room.difficulty,lv:room.lv});
  setTimeout(()=>spawnWave(room),150); // 150ms: enough for game:started to be processed client-side
...
io.on('connection',socket=>{
  ...
  socket.on('zombie:killed',data=>{
    ...
    if(r.killedCount>=(r.totalZombies||Infinity)&&r.gst==='playing'){
      r.gst='wave_clear';r.waveClearTimer=150;
