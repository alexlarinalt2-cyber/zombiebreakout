...
function gameTick(room){
  ...
  for(let[,p] of room.players){
    ...
    if(p.scd>0)p.scd--;
    let dx=0,dy=0;
    if(p.keys.w)dy--;if(p.keys.s)dy++;if(p.keys.a)dx--;if(p.keys.d)dx++;
    if(dx||dy){
      let sp=PSPD*(p.speedMult||1);
      let[nx,ny]=nrm(dx,dy);
      let[rx,ry]=resAll(p.x+nx*sp,p.y+ny*sp,PR);
      p.x=rx;p.y=ry;
    }
    if(p.shoot&&p.scd===0){
  ...
  for(let i=room.bl.length-1;i>=0;i--){
    let b=room.bl[i];b.x+=b.vx;b.y+=b.vy;b.l--;
    if(b.l<=0||bulletHitsWall(b.x,b.y)||b.x<-20||b.x>WW+20||b.y<-20||b.y>WH+20){room.bl.splice(i,1);continue;}
    let hit=false;
    for(let j=room.zs.length-1;j>=0;j--){
      let z=room.zs[j];if(z.dying)continue;
      if(dst([b.x,b.y],[z.x,z.y])<BR+ZT[z.type||'basic'].r){
        let shooter=room.players.get(b.owner);
        let dm=Math.max(1,Math.round(b.dmg*((shooter&&shooter.dmgMult)||1)));
        z.hp-=dm;z.flashT=10;z.hpBarTimer=120;
        let[kx,ky]=nrm(b.vx,b.vy);z.x+=kx*18;z.y+=ky*18;
        if(z.hp<=0)killZombie(room,j,kx*5,ky*5,b.owner);
        if(b.pierce>0){b.pierce--;}else{room.bl.splice(i,1);hit=true;break;}
      }
    }
  }

  // ── Zombies ────────────────────────────────────────────────────
  for(let i=room.zs.length-1;i>=0;i--){
    let z=room.zs[i];
    if(z.dying){
      z.x+=z.dyingVx;z.y+=z.dyingVy;z.dyingVx*=0.82;z.dyingVy*=0.82;
      if(--z.dyingTimer<=0)room.zs.splice(i,1);
      continue;
    }
    if(z.flashT>0)z.flashT--;
    if(z.hpBarTimer>0)z.hpBarTimer--;
    z.pulse=(z.pulse+0.09)%(Math.PI*2);
    if(z.staggerT>0)z.staggerT--;
    if(z.swingT>0)z.swingT=Math.max(0,z.swingT-0.06);

    let tgt=closestPlayer(room,z.x,z.y);

    // Zombie-zombie separation
    let sx=0,sy=0;
    for(let z2 of room.zs){
      if(z2===z||z2.dying)continue;
      let d2=dst([z.x,z.y],[z2.x,z2.y]),sepR=ZT[z.type||'basic'].r*3.2;
      if(d2<sepR&&d2>0){let w=(1-d2/sepR)**2;let[rx,ry]=nrm(z.x-z2.x,z.y-z2.y);sx+=rx*w*2.2;sy+=ry*w*2.2;}
    }

    let skip=false;
    if(z.type==='boss'){
      if(++z.chargeTimer>90&&!z.charging){
        z.charging=true;z.chargeTimer=0;
        let[cdx,cdy]=nrm(tgt.x-z.x,tgt.y-z.y);
        let ca=Math.atan2(cdy,cdx)+(Math.random()-0.5)*0.6;
        z.chargeDx=Math.cos(ca);z.chargeDy=Math.sin(ca);
      }
      if(z.charging){
        let[rx,ry]=resAll(z.x+z.chargeDx*z.sp*4.5,z.y+z.chargeDy*z.sp*4.5,ZT.boss.r);
        let mv=Math.hypot(rx-z.x,ry-z.y);z.x=rx;z.y=ry;
        if(++z.chargeTimer>60||mv<0.5){z.charging=false;z.chargeTimer=0;}
        skip=true;
      }
    }
    if(z.type==='runner'&&!skip){
      if(--z.zigzagTimer<=0){z.zigzagOffset=(Math.random()-0.5)*2.2;z.zigzagTimer=25+Math.random()*20;}
    }
    if(!skip&&z.staggerT<=0){
      let dToP=dst([z.x,z.y],[tgt.x,tgt.y]),mx=0,my=0;
      let[fdx,fdy]=getFlowDir(room.ff,tgt.x,tgt.y,z.x,z.y);
      if(z.type==='runner'){
        if(dToP<300){
          let[pdx,pdy]=nrm(tgt.x-z.x,tgt.y-z.y);
          let perp=[-pdy*z.flankSide,pdx*z.flankSide];
          mx=pdx*0.6+perp[0]*0.4;my=pdy*0.6+perp[1]*0.4;
          if(--z.flankTimer<=0){z.flankSide=Math.random()<0.5?1:-1;z.flankTimer=60+Math.random()*60;}
          if(z.zigzagOffset){let p2=nrm(-my,mx);mx+=p2[0]*z.zigzagOffset*0.4;my+=p2[1]*z.zigzagOffset*0.4;}
        }else{mx=fdx;my=fdy;}
      }else if(z.type==='exploder'){
        if(dToP>200){mx=fdx*0.6;my=fdy*0.6;}
        else{let[pdx,pdy]=nrm(tgt.x-z.x,tgt.y-z.y);mx=pdx*1.8;my=pdy*1.8;}
      }else{
        if(dToP<240&&!bulletHitsWall((z.x+tgt.x)/2,(z.y+tgt.y)/2)){
          let tx2=tgt.x+Math.cos(z.surroundAng)*(ZT[z.type||'basic'].r+PR+4);
          let ty2=tgt.y+Math.sin(z.surroundAng)*(ZT[z.type||'basic'].r+PR+4);
          let[sdx,sdy]=nrm(tx2-z.x,ty2-z.y);
          mx=fdx*0.55+sdx*0.45;my=fdy*0.55+sdy*0.45;
        }else{mx=fdx;my=fdy;}
      }
      mx+=(Math.random()-0.5)*0.12;my+=(Math.random()-0.5)*0.12;
      let[ndx,ndy]=nrm(mx+sx*0.5,my+sy*0.5);
      let[rx,ry]=resAll(z.x+ndx*z.sp,z.y+ndy*z.sp,ZT[z.type||'basic'].r);
      z.x=rx;z.y=ry;
    }

    // Attack closest player
    let atkR=ZT[z.type||'basic'].r+PR+10;
    if(dst([z.x,z.y],[tgt.x,tgt.y])<atkR&&tgt.alive){
      z.attackT++;
      if(z.attackT%38===0){z.swingT=1.0;z.swingSide*=-1;}
      if(z.attackT%38===19&&tgt.inv===0){
        let dmg=zDmg(room.lv,tgt.armor||0);
        tgt.hp-=dmg;tgt.inv=50;
        if(z.type==='tank'){let[kx,ky]=nrm(tgt.x-z.x,tgt.y-z.y);tgt.x+=kx*30;tgt.y+=ky*30;tgt.inv=80;}
        if(tgt.hp<=0){tgt.hp=0;tgt.alive=false;io.to(tgt.id).emit('player:died');}
      }
    }else z.attackT=0;
  }

  // ── Wave end ───────────────────────────────────────────────────
  if(room.zs.filter(z=>!z.dying).length===0){
    room.gst='wave_clear';room.waveClearTimer=150;
    io.to(room.id).emit('wave:clear',{lv:room.lv});
  }
...
function broadcastState(room){
  ...
  io.to(room.id).emit('state',{
    ...
    players,
    zombies:room.zs.map(z=>({
      id:z.id,x:z.x,y:z.y,hp:z.hp,mhp:z.mhp,type:z.type,
      dying:z.dying,dyingVx:z.dyingVx,dyingVy:z.dyingVy,
      flashT:z.flashT,hpBarTimer:z.hpBarTimer||0,
      charging:z.charging,pulse:z.pulse,surroundAng:z.surroundAng,
      swingT:z.swingT||0,swingSide:z.swingSide||1,staggerT:z.staggerT||0,attackT:z.attackT||0,
    })),
    bullets:room.bl.map(b=>({id:b.id,x:b.x,y:b.y,vx:b.vx,vy:b.vy,wtype:b.wtype})),
  });
}
